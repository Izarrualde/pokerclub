...
//            $rsm);
            ...
            $query = $this->_em->createNativeQuery('SELECT n0_.* FROM notifications n0_, users as users
    ...
    public function fetchCountUserNotifications($userId)
    {
        $rsm = new ResultSetMappingBuilder($this->_em);
        $rsm->addRootEntityFromClassMetadata($this->_entityName, 'n');
        $query = $this->_em->createNativeQuery('SELECT n0_.id FROM notifications n0_, users as users
